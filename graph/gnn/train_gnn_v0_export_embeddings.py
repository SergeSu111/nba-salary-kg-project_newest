...
# -*- coding: utf-8 -*-
"""
train_gnn_v0_export_embeddings.py

GNN V0 (GraphSAGE) for NBA salary KG (player-team-agent graph):
...
- Export learned player embeddings to: graph/embeddings/gnn_v0_sage_player_embeddings.csv
- (Optional) Export all node embeddings to: graph/embeddings/gnn_v0_sage_node_embeddings.csv

This is V0: structure-only node features (trainable embedding per node), GraphSAGE ignores relation types.
Train/Test split follows your Step2: train seasons < TEST_SEASON, test season == TEST_SEASON.

Run (from repo root):
  python scripts/train_gnn_v0_export_embeddings.py
"""
...
class Config:
    ...
    out_player_emb_path: Path = Path("graph/embeddings/gnn_v0_sage_player_embeddings.csv")
    out_node_emb_path: Path = Path("graph/embeddings/gnn_v0_sage_node_embeddings.csv")
...
def load_tabular_labels(tabular_path: Path, target_col: str) -> pd.DataFrame:
    df = pd.read_csv(tabular_path)
    required = {"player_id", "season", target_col}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Tabular file missing columns: {missing}. Found: {list(df.columns)}")

    df = df[["player_id", "season", target_col]].copy()
...
class GraphSAGERegressor(nn.Module):
    ...
    def forward(self, edge_index: torch.Tensor, node_idx: torch.Tensor) -> torch.Tensor:
...
@torch.no_grad()
def evaluate(model: nn.Module, edge_index: torch.Tensor, samples: pd.DataFrame, device: str) -> Dict[str, float]:
    model.eval()
    ...
    y_pred, _ = model(edge_index, node_idx)
...
def train_loop(cfg: Config) -> None:
    ...
    tab = load_tabular_labels(cfg.tabular_path, cfg.target_col)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if epoch % cfg.log_every == 0 or epoch == 1:
            tr = evaluate(model, edge_index, train_df, cfg.device)
            te = evaluate(model, edge_index, test_df, cfg.device)
            print(
            ...
            # Track best by test rmse (or r2 if you prefer)
            if te["rmse"] < best["rmse"]:
                best["epoch"] = epoch
                best["rmse"] = te["rmse"]
                best["state"] = {k: v.detach().cpu().clone() if torch.is_tensor(v) else v
